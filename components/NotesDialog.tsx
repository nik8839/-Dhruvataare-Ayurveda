...
export default function NotesDialog({ onClose }: NotesDialogProps) {
  ...
  const loadTaxonomy = async () => {
    try {
      setLoading(true)
      const response = await pdfAPI.getTaxonomy('notes')
      ...
      console.error('Error loading Notes taxonomy:', error)
    } finally {
      setLoading(false)
    }
  }
  ...
  if (loading) {
    return (
      <div className="fixed inset-0 bg-black bg-opacity-60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div className="glass rounded-2xl shadow-medical-lg max-w-2xl w-full p-8 border border-purple-100">
  ...
  return (
    <>
      <div className="fixed inset-0 bg-black bg-opacity-60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div className="glass rounded-2xl shadow-medical-lg max-w-2xl w-full p-8 border border-purple-100 relative overflow-hidden">
          <div className="absolute top-0 left-0 right-0 h-2 medical-gradient-purple"></div>
          <div className="flex items-center justify-between mb-8">
            <div>
              <h2 className="text-4xl font-bold bg-gradient-to-r from-purple-600 to-pink-600 bg-clip-text text-transparent">Notes</h2>
              <p className="text-gray-600 text-sm mt-1">Comprehensive Study Materials</p>
            </div>
            <button
              onClick={closeAll}
          ...
          {!hasContent ? (
            <div className="text-center py-8 text-gray-600">
              No notes content available yet. Check back soon!
            </div>
          ) : (
            <div className="space-y-4">
              {Object.entries(taxonomy).map(([yearKey, yearData], index) => (
                <button
                  ...
                  className="group w-full relative overflow-hidden medical-gradient-purple rounded-xl p-6 hover:shadow-medical-lg transform hover:scale-[1.02] transition-all duration-500 text-left"
          ...
          <div className="mt-8 pt-6 border-t border-gray-200 text-center">
            ...
            {!showSocials ? (
              <button
                ...
                className="flex items-center justify-center space-x-2 w-full medical-gradient-purple hover:shadow-medical-lg rounded-xl p-4 text-white font-semibold transition-all duration-300 transform hover:scale-105 shadow-medical"
              >
            ...
            <div className="grid grid-cols-2 gap-6">
              {taxonomy[selectedYear].subjects.map((subject, index) => (
                <button
                  ...
                  className="flex flex-col items-center justify-center p-6 bg-gradient-to-br from-purple-50 to-purple-100 rounded-full w-32 h-32 mx-auto hover:shadow-lg transform hover:scale-105 transition-all duration-300"
                >
      ...
      {showPaperDialog && selectedYear && selectedSubject && taxonomy[selectedYear] && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-[55] flex items-center justify-center p-4">
          <div className="bg-white rounded-lg shadow-2xl max-w-2xl w-full p-6">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-2xl font-bold text-gray-800">
                {selectedSubject} - Select Paper
              </h3>
              <button
                onClick={closeAll}
                className="text-gray-600 hover:text-gray-800"
